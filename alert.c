...
**                                                                           **
** ========================================================================= **
**     OPEN-SOURCE IMPLEMENTATION OF ORACLE DATABASE BUILT-IN DBMS_ALERT     **
** ========================================================================= **
  ...
  if (create) {
    if (first_free != NOT_FOUND) {
      ...
      ereport(ERROR,
        ...
        errhint(
          "There are too many collaborating sessions. Increase MAX_LOCKS in 'pipe.h'.")));
    }
  }
...
find_event (
  ...
  for (i = 0; i < MAX_EVENTS; i++) {
    if ((events[i].event_name != NULL) &&
      (textcmpm(event_name, events[i].event_name) == 0)) {
      if (event_id != NULL) {
        *event_id = i;
      }
      return (&events[i]);
  ...
  if (create) {
    ...
    ereport(ERROR,
      ...
      errhint(
        "There are too many collaborating sessions. Increase MAX_EVENTS in 'pipe.h'.")));
  }
...
dbms_alert_defered_signal (
  ...
  if (strcmp((relname = SPI_getrelname(trigdata->tg_relation)),
    "ora_alerts") != 0) {
    ereport(ERROR,
      (errcode(ERRCODE_TRIGGERED_ACTION_EXCEPTION),
  ...
  if (SPI_ERROR_NOATTRIBUTE ==
    (message_col = SPI_fnumber(tupdesc, "message"))) {
    ereport(ERROR,
      (errcode(ERRCODE_TRIGGERED_ACTION_EXCEPTION),
  ...
  if (ora_lock_shmem(SHMEMMSGSZ, MAX_PIPES, MAX_EVENTS, MAX_LOCKS, false)) {
    ...
    if (!(plan =
      SPI_prepare("DELETE FROM ora_alerts WHERE ctid = $1", 1, argtypes))) {
      ereport(ERROR,
        (errcode(ERRCODE_TRIGGERED_ACTION_EXCEPTION),
...
dbms_alert_signal (
  ...
  if (0 == SPI_processed) {
    SPI_EXEC("CREATE TEMP TABLE ora_alerts(event text, message text)",
      UTILITY);
