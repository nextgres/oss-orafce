...
**                                                                           **
** ========================================================================= **
**     OPEN SOURCE IMPLEMENTATION OF ORACLE DATABASE DATE/TIME FUNCTIONS     **
** ========================================================================= **
...
static const WeekDays WEEKDAYS[] =
{
  /* Japanese, UTF8 */
  { PG_UTF8,
                { "\346\227\245", "\346\234\210",  "\347\201\253",
                "\346\260\264",
                "\346\234\250", "\351\207\221", "\345\234\237"     } },
  /* Japanese, EUC_JP */
  { PG_EUC_JP,
                { "\306\374",     "\267\356",      "\262\320",     "\277\345",
                "\314\332",     "\266\342",     "\305\332"             } },
...
STRING_PTR_FIELD_TYPE ora_days[] =
{
...
_ora_date_round (
  ...
  switch (f)
  {
    CASE_fmt_CC
    if (y > 0) {
      result = DATE2J((y/100)*100+(day < DATE2J((y/100)*100+50, 1,
          1) ? 1 : 101), 1, 1);
    } else {
      result =
        DATE2J((y/100)*100+(day < DATE2J((y/100)*100-50+1, 1,
          1) ? -99 : 1), 1, 1);
    }
    break;
    CASE_fmt_YYYY
    ...
    CASE_fmt_Q
      result =
      DATE2J(y, ((m-1)/3)*3+(day < (DATE2J(y, ((m-1)/3)*3+2, 16)) ? 1 : 4), 1);
    break;
...
ora_to_date (
  ...
  if (nls_date_format && strlen(nls_date_format)) {
    ...
    result =
      DatumGetTimestamp(DirectFunctionCall1(timestamptz_timestamp, newDate));
  } else {
...
#define ROUND_MDAY(_tm_)                                            \
  do { if (rounded) { _tm_->tm_mday += _tm_->tm_hour >= 12 ? 1 : 0; \
       } } while (0)
