...
holiday_desc_comp (
  ...
  if (0 ==
    (result = ((holiday_desc *) a)->month - ((holiday_desc *) b)->month)) {
...
easter_holidays (
  ...
  if (use_great_friday || use_easter) {
    if ((m == 3) || (m == 4)) {
      ...
      easter_sunday =
        date2j(y, easter_sunday_month,
          easter_sunday_day) - POSTGRES_EPOCH_JDATE;

      if (use_easter &&
        ((day == easter_sunday) || (day == easter_sunday + 1))) {
        return (true);
      }
...
plvdate_set_nonbizday_dow (
  ...
  int d =
    ora_seq_search(VARDATA_ANY(day_txt), ora_days, VARSIZE_ANY_EXHDR(day_txt));
...
plvdate_set_nonbizday_day (
  ...
  if (arg2) {
    ...
    if (NULL !=
      bsearch(&hd, holidays, holidays_c, sizeof(holiday_desc),
      holiday_desc_comp)) {
      ereport(ERROR,
    ...
    if (NULL !=
      bsearch(&arg1, exceptions, exceptions_c, sizeof(DateADT),
      dateadt_comp)) {
      ereport(ERROR,
        (errcode(ERRCODE_DUPLICATE_OBJECT),
        errmsg("nonbizday registeration error"),
 ...
 */
Datum
plvdate_default_holidays (
  PG_FUNCTION_ARGS
) {
  text *country = PG_GETARG_TEXT_PP(0);

  country_id =
    ora_seq_search(VARDATA_ANY(country), states, VARSIZE_ANY_EXHDR(country));
  ...
  memcpy(holidays, defaults_ci[country_id].holidays,
    holidays_c*sizeof(holiday_desc));
