...
**                                                                           **
** ========================================================================= **
**   OPEN-SOURCE IMPLEMENTATION OF ORACLE DATABASE MISCELLANEOUS FUNCTIONS   **
** ========================================================================= **
  ...
  if ((locale_len > 0) &&
    ...
    *(locale_str + locale_len) = '\0';
    ...
    if (!setlocale(LC_COLLATE, locale_str)) {
      elog(ERROR, "failed to set the requested LC_COLLATE value [%s]",
        locale_str);
    }
  ...
  PG_CATCH();
  {
    if (changed_locale) {
      ...
      if (!setlocale(LC_COLLATE, lc_collate_cache)) {
        elog(FATAL, "failed to set back the default LC_COLLATE value [%s]",
          lc_collate_cache);
      }
    }
  }
