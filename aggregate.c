...
**                                                                           **
** ========================================================================= **
**     OPEN SOURCE IMPLEMENTATION OF ORACLE DATABASE AGGREGATE FUNCTIONS     **
** ========================================================================= **
...
orafce_median4_finalfn (
  ...
  qsort(state->d.float4_values, state->nelems, sizeof(float4),
    orafce_float4_cmp);
  ...
  if (lidx == hidx) {
    ...
    result = (state->d.float4_values[lidx] + state->d.float4_values[hidx]) /
      2.0f;
  }
...
orafce_median8_finalfn (
  ...
  qsort(state->d.float8_values, state->nelems, sizeof(float8),
    orafce_float8_cmp);
  ...
  if (lidx == hidx) {
    ...
    result = (state->d.float8_values[lidx] + state->d.float8_values[hidx]) /
      2.0;
  }
