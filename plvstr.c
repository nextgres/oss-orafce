...
plvstr_is_prefix_text (
  ...
  if (mb_encode && !case_sens) {
    str =
      (text *) DatumGetPointer(DirectFunctionCall1(lower,
        PointerGetDatum(str)));
    prefix =
      (text *) DatumGetPointer(DirectFunctionCall1(lower,
  ...
  for (i = 0; i < pref_len; i++) {
    ...
    if (case_sens || mb_encode) {
      ...
      if (pg_toupper((unsigned char) *ap++) !=
        pg_toupper((unsigned char) *bp++)) {
        break;
      }
    }
  }
