...
static char *
dbms_utility_format_call_stack (
  char mode
) {
  ...
  if (edata->context) {
    ...
    while (*start) {
      ...
      switch (mode)
      {
        case 'o':
          {
            appendStringInfo(sinfo, "%8x    %5s  function %s", (int) fnoid,
              line, oname);
          }
          break;
        ...
        case 'p':
          {
            appendStringInfo(sinfo, "%8d    %5s  function %s", (int) fnoid,
              line, oname);
          }
          break;
...
Datum
dbms_utility_format_call_stack0 (
  PG_FUNCTION_ARGS
) {
...
Datum
dbms_utility_format_call_stack1 (
  PG_FUNCTION_ARGS
) {
